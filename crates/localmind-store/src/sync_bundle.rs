...
/// versions).
pub const SYNC_BUNDLE_FORMAT_VERSION: u32 = 2;
/// Encrypted-envelope format version.
pub const ENCRYPTED_BUNDLE_FORMAT_VERSION: u32 = 2;
...
/// A sync cursor: the content version of each memory seen at its boundary.
/// Import keeps one per peer for idempotence; export keeps one snapshot cursor
/// so a formerly exported id disappearing from the syncable set is observable.
...
impl SyncCursor {
    ...
    /// an id present in the cursor but absent from `entries` produces one
    /// tombstone and is then removed from the advanced cursor.
    #[must_use]
    pub fn diff(
        &self,
        entries: &[MemoryEntry],
        ...
        for (memory_id, previous_version) in &self.versions {
            if present.contains(memory_id.as_str()) {
                continue;
            }
            let version = tombstone_version(memory_id, previous_version);
            advanced.versions.remove(memory_id);
            ops.push(SyncOp::new(
                OpKind::Tombstone,
                memory_id.clone(),
                version,
                own_device_label.to_string(),
                None,
            ));
        }
        (ops, advanced)
...
fn tombstone_version(memory_id: &str, previous_version: &str) -> String {
    digest_hex(format!("deleted:{memory_id}:{previous_version}").as_bytes())[..16].to_string()
}
...
mod tests {
    ...
    #[test]
    fn diff_emits_a_tombstone_once_when_an_exported_memory_disappears() {
        let original = entry("deleted", "obsolete guidance", vec![]);
        let (_, cursor) = SyncCursor::default().diff(&[original], "PC");

        let (ops, cursor) = cursor.diff(&[], "PC");

        assert_eq!(ops.len(), 1);
        assert_eq!(ops[0].kind, OpKind::Tombstone);
        assert_eq!(ops[0].memory_id, "deleted");
        assert_eq!(ops[0].origin_device, "PC");
        assert_eq!(ops[0].entry, None);
        assert!(!cursor.versions.contains_key("deleted"));

        let (again, _) = cursor.diff(&[], "PC");
        assert!(again.is_empty(), "a deletion must not be re-emitted");
    }

    #[test]
    fn tombstone_capability_is_versioned_and_v1_payloads_remain_readable() {
        assert_eq!(SYNC_BUNDLE_FORMAT_VERSION, 2);
        assert_eq!(ENCRYPTED_BUNDLE_FORMAT_VERSION, 2);

        let signing = SigningKey::from_bytes(&[21u8; 32]);
        let trusted = [signing.verifying_key().to_bytes()];
        let mut bundle = SyncBundle::new(Vec::new());
        bundle.format_version = 1;
        let signed = SignedSyncBundle::sign(bundle, &signing).unwrap();
        let recipient = EncryptionSecretKey::from([22u8; 32]);
        let mut encrypted =
            EncryptedBundle::seal_to_devices(&signed, &[device(&recipient, [23u8; 32])]).unwrap();
        encrypted.format_version = 1;

        let opened = encrypted.open(&recipient).unwrap();
        assert!(matches!(
            opened.verify(&trusted),
