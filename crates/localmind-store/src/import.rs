use crate::{
    MemoryPersistence, MemoryPersistenceError, ProjectConfig, Redaction, Redactor, StoreConfigError,
};
use localmind_core::{
    AuditEventKind, EvidenceKind, EvidenceRef, SessionId, SessionOutcome, SessionRecord,
    SessionSource,
};
...
impl TranscriptImporter {
    ...
    pub fn import_text(
        ...
        // The transcript and metadata are durable at this point. Audit only
        // non-content provenance: raw/redacted transcript text and paths never
        // enter the database metadata.
        let persistence = MemoryPersistence::open_project(&config.project_root)?;
        persistence.record_custom_audit(
            AuditEventKind::SessionImported,
            "localmind-import",
            session_id.as_str(),
            &serde_json::json!({
        ...
        persistence.record_custom_audit(
            AuditEventKind::TranscriptRedacted,
            "localmind-import",
            session_id.as_str(),
            &serde_json::json!({ "redaction_count": report.redactions.len() }),
        )?;
