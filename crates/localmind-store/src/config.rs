...
mod tests {
    ...
    fn discover_err(toml: &str) -> StoreConfigError {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join(CONFIG_FILE_NAME), toml).unwrap();
        ProjectConfig::discover(dir.path()).unwrap_err()
    }

    #[test]
    fn local_only_false_is_refused_so_loopback_only_is_the_only_reachable_state() {
        // `localmind_inference::EgressPolicy` defaults to `LoopbackOnly`, and the
        // reason that default is a *guarantee* rather than a convenience is this
        // rejection: there is no configuration in which `local_only` is false, so
        // there is no configuration that could ask for unrestricted egress.
        //
        // Pinned here because the two halves live in different crates. Without
        // this test, relaxing the rejection would silently widen what the
        // inference layer permits, and nothing would fail.
        assert!(matches!(
            discover_err(
                "[learning]
enabled = true
...
"
            ...
            StoreConfigError::RemoteLearningUnsupported { .. }
        ));
    }

    #[test]
    fn local_only_defaults_to_true() {
        let config = discover(
            "[learning]
enabled = true
",
        ...
        assert!(
            config.config.learning.local_only,
            "omitting the setting must not opt out of it"
        );
    }
