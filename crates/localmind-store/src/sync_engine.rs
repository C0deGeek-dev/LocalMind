...
//! is deliberately out of scope. Local deletions do produce one tombstone from
//! the last successfully exported snapshot; the peer only flags its memory for
//! review and never auto-deletes it.

use std::path::{Path, PathBuf};

use crate::{
    author_fingerprint, BundleScope, EncryptedBundle, KeyStore, MemoryBundleExporter,
    MemoryPersistence, ProjectConfig, ProjectIdentity, ReviewQueue, SignedSyncBundle, SyncCursor,
    SyncOp, TrustClass, VerificationOutcome,
};
use localmind_core::{
...
const EXPORT_CURSOR_FILE: &str = "export-cursor.json";
...
pub struct SyncStatus {
    ...
    /// The path-independent project key two machines must agree on for their
    /// exchanged memory to describe the same project.
    pub project_key: String,
    /// Where that key came from: `explicit`, `git_remote`, or `directory_name`.
    pub project_key_source: String,
    /// Whether the key agrees across machines without the user checking it. A
    /// directory-name fallback does not, so `sync status` says so.
    pub project_key_stable: bool,
...
impl SyncEngine {
    ...
    pub fn run(&self, folder: &Path) -> Result<SyncRunReport, SyncEngineError> {
        ...
        let export_cursor = self.load_export_cursor()?;
        let (delta_ops, advanced_export_cursor) =
            export_cursor.diff(&local_entries, &persistence.retrieval_device_label());
        let mut ops: Vec<SyncOp> = local_entries.iter().map(snapshot_op).collect();
        ops.extend(
            delta_ops
                .into_iter()
                .filter(|op| matches!(op.kind, crate::OpKind::Tombstone)),
        );
        ...
        if devices.is_empty() {
            ...
            // Advance only after the encrypted snapshot is durably published.
            // On a failed write, retrying the tombstone is safer than losing it.
            self.save_export_cursor(&advanced_export_cursor)?;
    ...
    pub fn status(&self, folder: &Path) -> Result<SyncStatus, SyncEngineError> {
        ...
        let pending_review = ReviewQueue::open_project(&self.project_root)?
            ...
            .unwrap_or(0);
        // The project key belongs in this report, not only in the config file:
        // it is the half of the exchange the user can still get wrong after
        // enrolment succeeds. Two machines with agreeing device identities but
        // disagreeing project keys sync bundles that describe different
        // projects, and nothing else in the run tells them so.
        let identity = ProjectIdentity::resolve(&ProjectConfig::discover(&self.project_root)?);
        Ok(SyncStatus {
            folder: folder.display().to_string(),
            own_fingerprint,
            project_key: identity.key,
            project_key_source: identity.source.as_str().to_string(),
            project_key_stable: identity.source.is_stable(),
    ...
    fn export_cursor_path(&self) -> PathBuf {
        self.sync_state_dir().join(EXPORT_CURSOR_FILE)
    }

    fn load_cursor(&self, peer: &str) -> Result<SyncCursor, SyncEngineError> {
        self.load_cursor_from(&self.cursor_path(peer))
    }

    fn save_cursor(&self, peer: &str, cursor: &SyncCursor) -> Result<(), SyncEngineError> {
        self.save_cursor_to(&self.cursor_path(peer), cursor)
    }

    fn load_export_cursor(&self) -> Result<SyncCursor, SyncEngineError> {
        self.load_cursor_from(&self.export_cursor_path())
    }

    fn save_export_cursor(&self, cursor: &SyncCursor) -> Result<(), SyncEngineError> {
        self.save_cursor_to(&self.export_cursor_path(), cursor)
    }

    fn load_cursor_from(&self, path: &Path) -> Result<SyncCursor, SyncEngineError> {
    ...
    fn save_cursor_to(&self, path: &Path, cursor: &SyncCursor) -> Result<(), SyncEngineError> {
        ...
        std::fs::write(path, json).map_err(|source| SyncEngineError::Folder {
            path: path.to_path_buf(),
            source,
        })
    }
