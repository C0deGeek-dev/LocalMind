...
//!
//! The resolved key and its source are reported by `localmind sync status`
//! (`SyncStatus::project_key*`), which is the whole of what the key does today:
//! it names the project for the people running an exchange, and never relocates
//! the store or routes an import (`D-LM-0040`).
