...
impl ChatEndpoint {
    /// Construct an endpoint under the default [`EgressPolicy::LoopbackOnly`].
    pub fn new(
        base_url: &str,
        model: &str,
    ...
    /// Construct an endpoint under an explicit egress policy.
    ///
    /// The policy is checked again at the moment of the request, not only here —
    /// see [`enforce_egress`]. Rejecting an unusable endpoint at construction is
    /// a courtesy so the failure names the config rather than a later request;
    ...
    pub fn with_policy(
        ...
        enforce_egress(base_url, policy)?;
...
impl EmbeddingEndpoint {
    ...
    /// The endpoint's base URL, for reporting *which* endpoint a capability
    /// state is about — a bare "unreachable" is not actionable without it.
    #[must_use]
...
fn post_json<T: Serialize>(
    ...
    // The last gate before the socket. Everything upstream is advisory.
    enforce_egress(url, policy)?;
...
/// Where an inference endpoint is allowed to send data.
///
/// LocalMind's `local_only` setting is an assertion that nothing leaves the
/// machine. It historically constrained the *storage* scope only, so a
/// configuration that read `local_only = true` would still ship prompt and
/// document text to any reachable `https://` inference host. This type is the
/// other half of that promise.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub enum EgressPolicy {
    /// The endpoint may only be contacted on a loopback address. This is the
    /// default, and it is what `local_only = true` means.
    #[default]
    LoopbackOnly,
    /// The endpoint may be contacted anywhere. Only reachable by explicitly
    /// constructing an endpoint with it; there is no configuration path here
    /// today, because `local_only = false` is itself rejected.
    Unrestricted,
}

/// The host portion of an `http(s)` URL, without port or credentials.
///
/// Deliberately small: it exists to answer "is this loopback", not to be a URL
/// parser. An input it cannot understand yields `None`, which the caller treats
/// as *not* loopback — unparseable must never be permissive.
fn endpoint_host(url: &str) -> Option<&str> {
    ...
    // Discard any `user:pass@` prefix; the host is what follows the last `@`.
    let authority = authority.rsplit('@').next()?;
    if let Some(closing) = authority.find(']') {
        // IPv6 literal: `[::1]:8080`.
        return authority
            .strip_prefix('[')
            .map(|value| &value[..closing.saturating_sub(1)]);
    }
...
///
/// **No name resolution happens here, and that is deliberate.** A hostname that
/// resolves to `127.0.0.1` when checked can resolve elsewhere when connected —
/// the check and the connection are separate lookups, and the gap between them
/// is exactly where DNS rebinding lives. Only literal loopback forms are
/// accepted, so the guarantee does not depend on a resolver's cooperation.
///
/// `0.0.0.0` and `[::]` are **not** loopback. They are bind-any addresses; as a
/// destination they route to a real interface.
#[must_use]
...
/// Refuse an outbound request that the policy does not permit.
///
/// This runs at the point of egress rather than at configuration load. Config
/// validation can be true when it runs and false when the request is made — a
/// file edited afterwards, a construction path that skips validation, a value
/// threaded through code that never saw the check. The request itself is the
/// only place the answer cannot be stale.
fn enforce_egress(url: &str, policy: EgressPolicy) -> Result<(), InferenceError> {
    if policy == EgressPolicy::Unrestricted {
        return Ok(());
    }
    match endpoint_host(url) {
        Some(host) if is_loopback_host(host) => Ok(()),
        Some(host) => Err(InferenceError::RemoteEndpointRefused {
            url: url.to_string(),
            host: host.to_string(),
        }),
        None => Err(InferenceError::RemoteEndpointRefused {
            url: url.to_string(),
            host: String::new(),
        }),
    }
...
pub enum InferenceError {
    ...
    #[error(
        "refusing to send to non-loopback inference endpoint {url} (host {host:?}):          LocalMind is local-only, so transcript- and document-derived text may only reach          a model server on this machine. Point the endpoint at localhost or 127.0.0.1,          or run the model locally"
    )]
    RemoteEndpointRefused { url: String, host: String },
