...
fn memory_status(project: &Path) -> Result<ToolOutput, ToolFailure> {
    ...
    ToolOutput::structured(json!({
        ...
        // Kept for existing consumers; `embedding_state` is the one that
        // distinguishes an unreachable endpoint from an absent one.
        "inference_configured": snapshot.inference_configured,
        "embedding_state": match &snapshot.embedding {
            localmind_store::EmbeddingCapability::NotConfigured => "not_configured",
            localmind_store::EmbeddingCapability::Unreachable { .. } => "unreachable",
            localmind_store::EmbeddingCapability::Healthy { .. } => "healthy",
        },
        "embedding_detail": snapshot.embedding.summary(),
        "memory_active": snapshot.memory_vectors.project_active
            + snapshot.memory_vectors.global_active,
        "memory_vectors": snapshot.memory_vectors.project_vectorized
            + snapshot.memory_vectors.global_vectorized,
        "memory_vector_holes": snapshot.memory_vectors.holes(),
        "memory_vectors_stale": snapshot.memory_vectors.stale,
...
fn doc_search(project: &Path, args: &Value) -> Result<String, ToolFailure> {
    ...
    match report.status {
        ...
        DocSearchStatus::EmbeddingEndpointUnavailable { error } => {
            return Ok(format!(
                "The embedding endpoint is configured but unreachable ({error}). Run \
                 `localbox embed-serve`, verify [inference] embedding_base_url + \
                 embedding_model in .localmind.toml, and retry."
            ));
        }
