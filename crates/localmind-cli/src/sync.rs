...
pub fn status(project: &Path, folder: Option<PathBuf>, json: bool) -> Result<()> {
    ...
    println!(
        "Project key:      {} ({})",
        status.project_key, status.project_key_source
    );
    if !status.project_key_stable {
        println!("                  weak — derived from this folder's name, so a peer agrees");
        println!(
            "                  only by coincidence. Pin `[sync] project_key` on each machine."
        );
    }
