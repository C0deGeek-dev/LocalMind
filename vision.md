...
| Retrieval — keyword + graph (§6) | implemented — FTS5/bm25 accepted-memory retrieval is host-wired through `localmind-search::hybrid_memory_search`; the graph-aware workspace blend is a tested engine-library surface | `localmind-search`, `localmind-store` |
| Retrieval — semantic/vector (§6) | implemented and host-wired — exact cosine over schema-versioned f32 BLOB rows; LocalPilot consumes the engine-owned keyword-floor + windowed RRF path when embeddings and reranking are enabled | `localmind-store`, `localmind-search` |
...
| Skill maintenance (§8) | implemented (manual lifecycle only) — retirement state, source-memory provenance, and audit rows; automatic refresh/staleness scanning remains a future feature | `localmind-store` |
