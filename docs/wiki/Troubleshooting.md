...
## Embeddings are configured but unavailable

Run `localbox embed-serve`, then verify `[inference] embedding_base_url` and
`embedding_model` in `.localmind.toml`. `localmind backfill` stops with this
repair cue when rows need vectors. Ingest, promotion, review-mode processing,
insights, MCP, and the UI keep their lexical/no-vector behaviour and say that
they degraded.

If LocalPilot started the configured server, LocalMind automatically leases the
exact registered server PID for the command lifetime. It does not start or stop
LocalBox. A reachable endpoint without that ownership record is treated as
user-managed: LocalMind uses it without creating a lease or claiming ownership.
