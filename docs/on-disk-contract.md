...
behaves exactly as before. `project_key` names the **path-independent project
identity**, so the same repo at `D:\repos\X` on one machine and `C:\repos\X` on
another can be confirmed to be the same project; when unset it is derived from
the git `origin` remote (HTTPS and SSH URLs normalize to the same `host/path`
key), and failing that from the project directory name (a weak fallback that can
collide). `localmind sync status` prints the resolved key, its source, and a
warning when the source is that weak fallback. Surfacing is what the key does
today: it does not move the project store, which is always the project directory
addressed by path, and it is not carried in an op-bundle, so it does not gate
import (`D-LM-0040`). `device_label` is the machine name stamped on synced
...
A `CandidateLesson` also carries two optional `#[serde(default)]` fields written
inside the same JSON blob, so neither needed a schema migration.

`hindsight` holds the evidence-linked `HindsightDraft` a candidate came out of,
when one was drafted: intended and observed outcome, bounded causal hypotheses
citing the candidate's own `evidence` entries by id, missed signals,
intervention, counterfactual prediction, applicability, preconditions,
invalidation, a concise proposed lesson, and an **advisory** `suggested_outcome`.
...
### Reported capability, not assumed capability

An embedding endpoint has three states a user needs told apart: not configured,
configured but not answering, and healthy. A bare configured/not-configured flag
collapses the middle one — which is the state that leaves every semantic path
degraded while the store itself stays perfectly healthy, and therefore the state
that can accumulate unnoticed for months.

The status snapshot probes rather than assumes, and reports:

- the **embedding capability** (`not configured` / `DEGRADED - <endpoint> …` /
  `healthy (<endpoint>)`). A degraded endpoint never flips store readiness: the
  lexical paths are untouched and must keep reporting healthy, or a legitimately
  offline user is told their store is broken.
- **memory-vector coverage** — active accepted memory per scope, how much of it
  carries an embedding, and how many vectors belong to rows that are no longer
  active. Counted over `status = 'active'` rows on both sides deliberately:
  deriving the gap as `total - vectors` is wrong whenever a superseded row's
  vector was never pruned, because the two errors cancel and the number still
  looks right.

The chat and embedding endpoints are reported separately. They are configured
independently, so describing an embedding-only configuration as a configured chat
endpoint invents one the user never set.

### Machine-global embedding leases

An embedding-capable LocalMind CLI lifetime may join a server already owned by
LocalPilot. The neutral state is under `~/.local-llm/embed-leases/`:

- `started-by-localpilot` is schema-1 JSON naming an owner label, normalized
  loopback socket, exact server PID, and `active`/`stopping` phase;
- `<client-pid>-<process-local-sequence>.lease` contains the client PID; and
- `lease-state.lock` serializes marker, stale-prune, acquisition, and last-release
  transactions across products.

The former plain-text `localpilot` marker remains readable only as legacy owner
state. LocalMind never trusts it. LocalPilot may migrate it after independently
matching LocalBox's runtime state to the configured endpoint and live server PID.
An invalid, stale, mismatched, or absent marker makes a reachable endpoint
user-managed: it can still serve configured inference, but LocalMind writes no
lease and gains no teardown rights. Lease drop removes only that unique lease;
only the process owner may act on a last-lease `stopping` token.

A diagnosed memory vector scan reports the same distinctions for retrieval:
embeddings not configured, endpoint unavailable, no memory vectors yet, an index
embedded under a different model, or a scan that ran.

### Scanning one `subject_kind`

`vector_index` holds every kind in one table, and the ranked scan is a single
pass over it. A caller that wants one kind must say so **inside** the scan
(`vector_scan_for_kind`) rather than taking a shared top-k and filtering the
result: the shared window is consumed by whichever kind dominates the index, so
the post-filter returns a fraction of what the caller expects. This is not a
theoretical ordering nicety — a real store carrying 8,079 `doc` vectors against
200 `memory` vectors leaves a top-64 with a median of one or two memory rows for
a documentation-shaped query.

The kind-scoped scan also reports what an empty result *means*: how many
candidate rows the kind holds, and the distinct models those rows were embedded
under. "No vectors of this kind" and "vectors exist but none is comparable with
this query" are different states, and a caller that cannot tell them apart
reports a model or dimension change as an empty index.
...
Schema upgrades follow the same concurrency contract. An already-current open
uses a lock-free `user_version` fast path; a stale opener takes an immediate
write transaction and re-reads the version under that lock before applying any
step. Concurrent stale openers therefore serialize through the busy timeout:
one applies the ladder, and each waiter observes the committed version instead
of replaying non-idempotent schema changes.
...
- **Op-bundle** (`SyncBundle`, `format_version` 2): `{ format_version, ops }`
...
- **Encrypted bundle** (`EncryptedBundle`, `format_version` 2) — **the only thing
  ...
  name is the exporting device's stable Ed25519 signing fingerprint. It reveals
  which pseudonymous enrolled device wrote the file to anyone who can list the
  sync folder, but no memory title, id, or count. A recipient `fingerprint`
  identifies which sealed copy is whose (the recipient's signing fingerprint);
  a non-recipient device cannot open any copy. Residual, documented leakage: the
  exporting device identity, number of recipients, and ciphertext size/timing.

**Cursors.** `export-cursor.json` records the ids and content versions in the
last successfully published syncable snapshot. Current entries are still sent
as the full snapshot; an id in that cursor but absent from the current snapshot
emits one payload-free `tombstone`, then leaves the advanced cursor so the next
run does not re-emit it. Each `cursor-<peer-fingerprint>.json` independently
records versions already imported from that peer, preventing duplicate review
work. Derived state (vectors, graph, usage) is never in an op-bundle; the
destination re-embeds locally.

Tombstone production starts with sync op-bundle **v2** and encrypted envelope
**v2**. The envelope bump is the rolling-upgrade gate: builds that predate
tombstone import reject the newer envelope before decrypting or attempting to
deserialize an unknown `OpKind`. A v2 reader continues to accept historic v1
envelopes and op-bundles. All enrolled peers must therefore support v2 before a
v2-producing device is introduced.
...
Each device writes exactly one `<device-fingerprint>.sync` file — a full-snapshot
...
<project root>/.localmind/sync/
  export-cursor.json                    # last successfully exported snapshot
...
- Sync op-bundle / encrypted envelope: `format_version` (currently **2**; v1 remains readable).
