...
The configured embedding endpoint remains a generic OpenAI-compatible endpoint;
LocalMind must not infer that it is LocalBox. When a reachable endpoint has the
machine-global schema-1 ownership record for that normalized loopback socket and
exact live server PID, an embedding-capable CLI command acquires a unique RAII
lease for its complete one-shot or server lifetime. A reachable endpoint with no
valid matching record is user-managed: it is usable, but never leased or claimed.
An unreachable endpoint cannot create state.

The canonical filesystem transaction lives in `localmind-inference`, not either
product CLI. An exclusive file lock covers stale PID pruning, acquisition,
release, and the last-lease transition to `stopping`; that transition closes the
join-versus-shutdown race. Drop only removes the caller's unique lease. The
process owner receives the last-release token and remains solely responsible for
start/stop effects and for confirming teardown. Legacy plain-text ownership is
not enough for LocalMind to join; only the owner may migrate it after verifying
the external runtime PID and endpoint.

The CLI lifetime matrix is explicit: ingest docs, promotion, review-mode,
insights, MCP serve, and UI are best-effort and keep their lexical/no-vector
floor; non-dry backfill acquires only when its plan contains work and fails with
the exact config plus `localbox embed-serve` repair cue. Status, lexical memory
search, dry-run backfill, and other non-embedding commands take no lease.

## D-LM-0040 — The project key is a surfaced sync precondition, not a store location

`[sync] project_key`, the git-remote derivation behind it, and the
directory-name fallback all resolve through `ProjectIdentity`. Nothing called it.
The key was parsed, validated, and then ignored — the same defect LocalHub#87
names for `foreign_env_weight`, in the same `[sync]` section.

`localmind sync status` now reports the resolved key, the source it came from,
and whether that source agrees across machines without the user checking.
`ProjectIdentitySource::is_stable` makes that judgement and is the only place it
is made; a directory-name key is printed with the warning that names the config
key which fixes it. This is the half of the exchange a user can still get wrong
after enrolment succeeds, and it was the half nothing reported.

The surface is deliberately no larger than that. The project key names the
project for the people running the exchange. It does not relocate the project
store, which remains the project directory addressed by path, and it is not
carried inside an op-bundle, so it does not gate or route import. Making the key
routing-relevant would change the wire format and needs its own decision.

Deleting instead was rejected in both available sizes: removing `is_stable`
alone leaves `ProjectIdentity::resolve` unreachable and the defect intact, and
removing the module means withdrawing a documented, user-facing config key that
projects may already have set.

- **Date**: 2026-08-27
- **Status**: accepted

## D-LM-0039 — Successful sync snapshots retain just enough history to emit one review-gated tombstone

The stable per-device sync file remains a full snapshot of present syncable
memory. Alongside it, `export-cursor.json` records the ids and content versions
of the last successfully published snapshot. `SyncCursor::diff` now treats an id
present there but absent from the current syncable set as a deletion: it emits a
payload-free `Tombstone`, removes the id from the advanced cursor, and therefore
does not emit it on the following unchanged run. The cursor advances only after
the encrypted file is published; a failed publish may retry a tombstone rather
than silently lose it.

Import keeps the D-LM-0016 boundary: a tombstone only flags an existing active
memory for review. It never deletes or supersedes the peer's copy. A recreated
memory is a new create after its prior id left the export cursor.

This is a wire-capability change even though `Tombstone` was already declared.
Both the signed op-bundle and encrypted envelope move to format v2. Bumping the
outer envelope is deliberate: a pre-tombstone reader refuses v2 before decrypting
and deserializing the inner enum, while v2 readers remain able to consume v1.
Operators must upgrade all enrolled peers before introducing a v2 producer.

- **Date**: 2026-08-27
- **Status**: accepted

## D-LM-0038 — A merge suggestion becomes an explicit review decision with a durable target

`SuggestedAction::MergeIntoExisting` was live extraction vocabulary, while
`ReviewAction::MergeInto` had only consumers: no reviewer surface constructed
it. The CLI now exposes `review merge`. By default it converts the candidate's
`duplicate_of` annotation into the target `ReviewItemId`; `--target` lets a
reviewer choose another existing item explicitly. The store rejects missing and
self-referential targets.

A merge closes the source item as `ReviewState::Merged` and records both the
stable `"merge"` action label and its target. Schema v12 adds nullable
`review_items.merge_target`; nullable is intentional because historic rows may
already carry `reviewer_action = "merge"` without having stored the target.
Those rows remain readable, while new decisions preserve enough information to
explain where the item went. The review-decision audit metadata carries the same
target without copying candidate content.

- **Date**: 2026-08-27
- **Status**: accepted

## D-LM-0037 — Every declared ingest-side audit kind has a producer; audit metadata carries provenance, never generated content

The six ingest-side `AuditEventKind` variants were durable wire vocabulary but
had no producer. The operations themselves shipped, leaving imports, redaction,
summaries, candidate extraction, and batch insights absent from the history that
is meant to explain how reviewable and accepted memory came to exist.

All six remain in the wire schema and gain producers at the operation that first
makes their subject durable:

| Kind | Producer | Cardinality |
|---|---|---|
| `SessionImported` | successful transcript import after metadata is written | one per imported session |
| `TranscriptRedacted` | successful transcript import after the sanitised transcript is written | one per imported session, including zero-redaction passes |
| `SummaryCreated` | successful session closeout after summary, candidates, and queue writes succeed | one per closeout |
| `CandidateLessonCreated` | successful session closeout | one per extracted candidate |
| `DistillationCreated` | successful distillation batch after its derived record is written | one per stored insight |
| `ResearchInsightCreated` | successful research batch after its derived record is written | one per stored insight |

Subjects are stable session or candidate ids. Metadata may contain source/format,
counts, and batch/session provenance, but never transcript text, summary text,
candidate text, model output, or filesystem paths. Tests execute each operation,
assert its event, and pin that generated content is absent from metadata. No kind
is retired, so historical deserialization needs no fallback or migration.

- **Date**: 2026-08-27
- **Status**: accepted

## D-LM-0036 — Skill-draft refresh remains future work; an inert counter is not a maintenance capability

`SkillDraftStore::refresh_from_memory` looked like the refresh half of skill
maintenance but only counted active records with non-empty source ids. It wrote
nothing, inspected no source memory, changed no staleness state, emitted no
audit event, and had no caller. Keeping that public symbol would preserve a
false capability boundary: a future caller could report successful refreshes
while leaving stale skill evidence active.

The function is removed and the implementation table now describes only the
manual lifecycle that actually ships: source-memory provenance, explicit
retirement, and audited state changes. Automatic skill refresh/staleness remains
the future feature already named in `vision.md`.

A future implementation must make its state transition and trigger explicit —
including what superseded or deleted source memory does to an active skill — and
must prove the mutation with an acceptance test before the status table calls it
implemented. A no-op scan or record count is not a compatible substitute.

There is no compatibility cost: the workspace-internal method had no caller or
serialized/on-disk contract, and this decision does not alter existing skill
records.

- **Date**: 2026-08-27
- **Status**: accepted

## D-LM-0035 — Staleness flagging is symmetric: both directions reach both stores, and clearing is audited

A memory flagged as a staleness candidate is **excluded from the queryless
context primer** — `rank_and_render` skips `stale_candidate` — while remaining
fully visible to keyword search. That asymmetry is deliberate and fine. What was
not fine is that the flag was effectively **one-way**.

`clear_stale_candidate` had no caller in any surface: no CLI, no MCP, no UI, not
even a test. It was also scoped to the project store alone, as was
`flag_for_review`, while the freshness pass reached the global store through a
private connection-scoped path. The net effect: a freshness pass could flag
dozens of global lessons, each one silently dropping out of the primer, and no
public API could flag or unflag a global memory at all.

Both directions now span the project and global stores, matching
`list_stale_candidates`, and clearing writes a `MemoryFlagCleared` audit row.

The audit row is the part worth insisting on. Flagging was already audited;
clearing was not, so a lifted flag left no trace and was indistinguishable from
one never set. A lifecycle that can only be read in one direction cannot be
reviewed.

- **Date**: 2026-08-24
- **Status**: accepted

## D-LM-0034 — `local_only` constrains inference egress, not only storage scope, and it is enforced at the request

`local_only` is LocalMind's assertion that nothing leaves the machine. It
constrained only the *storage* scope: a configuration reading
`local_only = true` was accepted alongside an `embedding_base_url` pointing at
any reachable host, and prompt, document and memory text would be sent there.
A setting whose name overstates its guarantee is itself a defect, so the
setting is **redefined to mean what it says** rather than renamed to match the
narrower behaviour.

An inference endpoint now carries an `EgressPolicy`. The default,
`LoopbackOnly`, permits only literal loopback destinations — `localhost` and
the RFC 6761 `.localhost` TLD, `127.0.0.0/8`, and `::1`. `0.0.0.0` and `[::]`
are refused: they are bind-any *listen* addresses, and as a destination they
route to a real interface.

**No name resolution is performed, deliberately.** A hostname that resolves to
`127.0.0.1` when checked can resolve elsewhere when connected — the check and
the connection are separate lookups, and the interval between them is where DNS
rebinding lives. Only literal forms are accepted, so the guarantee does not
depend on a resolver's cooperation. The cost is that a hostname alias for the
local machine is refused; that is the intended trade, and the refusal names the
host and says what to do instead.

**The check runs at the request, not at configuration load.** Validation that
runs once can be true when it runs and false when the request is made: a file
edited afterwards, a construction path that skips validation, a value threaded
through code that never saw the check. Construction still rejects an unusable
endpoint, but that is a courtesy so the failure names the configuration rather
than a later request — it is not the guarantee.

`EgressPolicy::Unrestricted` exists as a seam and is not reachable from
configuration. It cannot become reachable by accident: `local_only = false` is
rejected outright (`RemoteLearningUnsupported`), so there is no configuration
in which the loopback constraint is off. That cross-crate link is pinned by a
test, because the two halves live in different crates and relaxing the
rejection would otherwise silently widen what the inference layer permits.

**Compatibility.** A configuration pointing inference at another machine — a
model server on the LAN, a hostname alias, a tunnelled remote — stops working
and reports why. That is the point of the change rather than a side effect:
such a configuration was already outside what `local_only` claimed, and it was
silent about it.
...
3. **Signed, then sealed, incremental op-bundles.** A payload is a set of
   ...
   **same** Ed25519 signer as the portable bundle (the shared logic is extracted,
   ...
   produced, so the folder only ever holds ciphertext. Each exporting device
   owns one stable file named by its Ed25519 signing fingerprint and atomically
   overwrites that full snapshot on every run. Op identity is content-addressed
   and conflicts route to review, so the version stamp is a content fingerprint,
   not a causal counter (a vector clock would be an optimization the
   never-auto-delete posture does not need). The crypto crate is pinned pure-Rust
   within MSRV 1.82; no bespoke crypto.
...
content does not. The stable filename also reveals which pseudonymous enrolled
device wrote a bundle to anyone who can list the folder; conflict reconciliation
is human, not automatic. See
`docs/on-disk-contract.md` for the storage contract.

> **Amended 2026-08-27 — the sync filename is the exporting device's signing
> fingerprint, not a ciphertext content address.** One stable name is part of
> the exchange shape: it lets a device replace its full snapshot atomically and
> lets peers exclude their own bundle without a manifest or reaper. Content
> addressing would create a new artefact whenever ciphertext changes and would
> therefore require retention, supersession, and cleanup semantics that this
> design does not have. The filename exposes the pseudonymous writer identity to
> the folder transport; memory titles, ids, and counts remain absent. The unused
> `EncryptedBundle::content_address` helper is removed so the code cannot imply
> the rejected contract.

> **Amended 2026-08-27 — foreign-environment ranking compares persisted device
> labels at the engine search boundary.** The live search index carries
> `origin_device`, not a second full `EnvFingerprint`, while `ProjectConfig`
> supplies the retrieving device label and `foreign_env_weight`. Consequently,
> `hybrid_memory_search` owns the one comparison used in production: a non-empty
> unequal origin label multiplies the final score by the configured weight, and
> `1.0` restores the unweighted order. The unused
> `EnvFingerprint::is_same_machine` method is removed rather than preserving a
> competing OS/architecture comparison that the ranking path cannot execute.
> An integration test reads both weight values through `ProjectConfig` and
> asserts their final orders, binding configuration to host-consumed retrieval.

> **Amended 2026-08-27 — deletion history is the export snapshot cursor, and
> tombstone-capable wire payloads are v2.** The stable device artefact continues
> to contain a full snapshot of present syncable memory. Its local
> `export-cursor.json` remembers the last successfully published ids so an id
> that disappears produces exactly one tombstone; inbound per-peer cursors keep
> their separate import-idempotence role. D-LM-0039 records the retry posture,
> review-only destination behavior, and the op-bundle plus envelope v2 rollout
> gate.
...
> **Amended 2026-08-24 — how the reference host actually resolved it.**
> LocalPilot exercised the "host decision" clause above and did **not** consume
> `rerank_scored`. Its ADR-0163 (refining ADR-0110) fuses the keyword and dense
> rankings with Reciprocal Rank Fusion in its own `fuse.rs`, which ranks on
> *position* rather than reordering a blend by precomputed cosines — a different
> algorithm, not a different call site, so consuming `rerank_scored` was not
> available to it without discarding ADR-0110.
>
> Two things follow, and both are recorded here rather than left implicit:
>
> 1. **`rerank_scored` has no production caller and is disclosed as
>    library-only.** It is not dead code to delete: this decision built it
>    deliberately, the contract holds, and a host that wants windowed
>    cosine-reorder semantics can still consume it. But no host does today, and
>    a reader should not infer one from this decision's text.
> 2. **The `rerank_window` contract above still binds the host**, and LocalPilot
>    now honours it: only the leading `rerank_window` hits may move and the tail
>    keeps keyword order. It had been reading the key and discarding it,
>    reordering the whole candidate list. Removing the key host-side was
>    considered and rejected — this decision ships and documents it, and the
>    engine's own entry points still honour it, so removal would have left one
>    config key meaning two different things at its two ends.

> **Amended 2026-08-26 — the non-rerank memory-search surface is now host-wired.**
> LocalPilot's production `context_hits` path now depends on
> `localmind-search` and calls `hybrid_memory_search`; the host-owned RRF module
> and its duplicate keyword/dense fusion were removed. To preserve LocalPilot
> ADR-0110/0163 rather than replacing one drift with another, the engine entry
> point now owns that exact contract: BM25 is the candidate floor, a kind-scoped
> dense scan supplies cosines, Reciprocal Rank Fusion uses `k = 60`, cosine
> breaks fused ties, and `rerank_window` bounds movement. The same boundary also
> applies `foreign_env_weight`, so D-LM-0027's foreign-machine down-weight is no
> longer stranded outside every host. Language filtering and the evaluation-only
> coverage-gate seam also pass through the engine boundary.
>
> `rerank_scored` remains library-only exactly as recorded in the 2026-08-24
> amendment: it implements cosine-slot reranking, not RRF. The workspace/graph
> search entry points likewise remain reusable engine APIs, but the crate itself
> now has a shipped dependency edge and its accepted-memory fusion path has one
> production implementation. LocalPilot carries a dependency/decision contract
> test that allows exactly its adapter crate to consume `localmind-search`; a
> consumer being added or removed therefore requires this decision to move too.
...
work — **D009 (2026-06-17): the ingest-embedding gap was abandoned** on YAGNI /
no-evidence grounds (the D005 evidence bar — a fixture
